...
`data` 文件夹里存放的是程序自动下载的 Omniglot 数据集。

Omniglot 是小样本学习中常用的数据集，里面主要是不同语言或符号系统中的手写字符图片。它的特点是类别很多，但每个类别的样本数量不算多，所以很适合做 Few-Shot Learning 小样本图像分类。

在我们的实验里，程序会从 `data` 文件夹中读取 Omniglot 图片，然后随机构造 5-way 5-shot 任务。
...
这些数据说明 Omniglot 中包含大量字符类别，可以支持小样本分类实验。
...
PPT 位置建议：5-way 5-shot 任务采样页，和 support set 放在一起讲。
...
---
 ...
 10. `10_powershell_final_accuracy.png`

中文名：5-way 5-shot 最终测试结果

这张图是 PowerShell 中最终测试结果的截图。

它展示了最终的平均测试准确率，例如：

平均准确率：97.77%

这是实验结果中最重要的截图，用于证明我们的 Prototypical Network 在 Omniglot 5-way 5-shot 任务上取得了较好的分类效果。
...
1. 实验目标：5-way 5-shot 小样本图像分类
2. 数据集介绍：Omniglot 数据集
3. 环境说明：Python、PyTorch、Torchvision
4. 数据样例：单张 Omniglot 图片
5. 任务采样：Support Set 和 Query Set
6. 采样验证：support 和 query 的维度输出
7. 模型方法：Prototypical Network 原型网络
8. 训练过程：Episode 训练日志
9. 实验结果：Loss 曲线、Accuracy 曲线、最终准确率
10. 总结：原型网络适合处理小样本分类任务，最终准确率较高
